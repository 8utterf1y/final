...
flowchart TD
    ...
    F --> G["披露 Memory、Knowledge、Experience、Skills 和 Agent 索引"]
    G --> H["合并内置工具和 MCP 工具"]
    H --> I["模型生成文本或工具调用"]
    I --> J{"是否有工具调用"}
    J --> K["无工具调用：回复用户"]
    ...
    S --> I
    K --> T["经验沉淀、Memory 与 Knowledge 持久化"]
```

这张图里有两条循环。第一条是 Agent Loop 的执行循环：模型输出、工具调用、工具执行、结果回填、再调用模型。第二条是长期复用循环：任务执行产生轨迹，轨迹被提炼成带 Description 和 Content 的经验；后续任务先用 Description 检索经验，再按 ID 读取完整 Content。Memory 和 Knowledge 则分别提供长期事实与外部文档证据。
...
System Prompt 是模型行为的底座，但这个项目没有把所有信息都硬塞进一段大 prompt。它把信息按生命周期分层：稳定的身份、工程规范、安全原则放在静态 system prompt；cwd、平台、shell、Git，以及 Memory、Knowledge、Experience、Skills 和自定义 Agent 的轻量索引放在动态上下文；项目级 `CLAUDE.md`、rules 和日期等内容通过 user context reminder 注入首条用户消息。知识正文和完整经验只在工具命中后作为 tool result 进入消息历史。

这层最重要的设计思想是渐进式披露。Memory 只注入少量索引和相关长期事实；Knowledge 先披露文档 manifest，需要时用 `knowledge_search` 返回相关片段；Experience 先披露经验 ID、标题和少量 Description，需要时用 `experience_search` 召回候选，再用 `experience_show` 读取完整 Content；Skill 不一开始注入完整 `SKILL.md`，只注入名称、描述和 `when_to_use`；自定义 Agent 不注入完整 prompt，只注入类型名和描述；MCP 则在首次 chat 连接后把外部工具 schema 注入工具列表。
...
flowchart TD
    ...
    C --> E["Git / cwd / shell"]
    C --> F["Memory 摘要和召回"]
    C --> G["Knowledge 文档 manifest"]
    C --> H["Experience 卡片 manifest"]
    C --> I["Skills 名称和描述"]
    C --> K["Custom Agent 类型描述"]
...
Skill fork 和 Sub-agent 是预防型上下文治理。它们在复杂任务开始前就把中间过程放到子 Agent 内部，主 Agent 只接收最终摘要。上下文压缩是事后治理，处理已经进入主上下文的内容。Memory 召回、Knowledge 片段和完整 Experience 都会占用上下文；如果不限制召回数量并按需披露，私有知识增强反而会挤占代码、工具结果和用户需求的空间。
...
短期上下文不是长期记忆。上下文会被 snip、compact、清理，也可能因为新会话而消失。长期复用要靠三类独立持久化：Memory 保存稳定偏好、反馈和项目事实；Knowledge/RAG 保存规范、接口和设计文档等事实型资料，按结构分块后返回局部证据；Experience 从任务轨迹里提炼过程型知识，完整流程保存为 Markdown，其中 Description 用于检索召回，Content 保存完整排查、修改和验证过程。

这三者的边界也要讲清楚。Memory 回答“这个用户或项目有什么长期偏好和事实”；Knowledge 回答“外部资料里有什么依据”；Experience 回答“过去类似任务是怎么做成的”。Knowledge 命中后直接返回相关 chunk；Experience 命中后只返回候选 description 卡片，模型还要调用 `experience_show(id)` 读取全文并核对适用条件。Skill 则回答“以后遇到这类任务应固定按什么流程做”。高质量经验可以进一步人工固化成 Skill，但当前项目不自动完成这一步。

```mermaid
flowchart TD
    A["当前任务执行"] --> B["TaskJournal 记录工具调用和结果"]
    B --> C["Experience 提炼结构化经验"]
    C --> D["完整 Markdown: Description + Content"]
    D --> E["experience_search 检索 Description"]
    E --> F["experience_show 按 ID 读取 Content"]
    F --> G["核对当前代码并验证"]
    G --> A
...
面试时可以说：上下文是短期工作区；Memory、Knowledge 和 Experience 是三类用途不同的长期资产。Experience 既包含从执行轨迹到结构化经验的生产机制，也包含“Description 召回、Content 全文读取、当前任务验证”的消费机制。
...
5. Prompt 层准备静态 system prompt，以及包含环境信息和各类轻量索引的动态上下文。
6. 模型流式生成文本或工具调用。
7. 如果有工具调用，先进入权限系统。
...
11. 模型继续下一轮，直到不再调用工具。
12. 主 Agent 输出最终答复。
13. 用户需要时显式保存经验；Memory、Knowledge 按各自入口独立持久化。
...
sequenceDiagram
    ...
    U->>CLI: 输入请求
    CLI->>A: 普通请求进入 chat
    A->>A: 准备 prompt / tools / memory / knowledge / experience index
    A->>M: 发送消息和工具 schema
    M->>A: 返回文本或工具调用
    A->>P: 检查工具调用
    ...
    M->>A: 最终回答
    A->>CLI: 输出给用户
    A->>S: Experience、Memory、Knowledge 分别沉淀
```
...
>
> 执行过程中，上下文管理控制 token 水位。大工具结果先持久化，只返回预览；旧工具结果可以 snip；高水位才 compact。Skill fork 和 Sub-agent 是事前隔离，避免复杂任务把中间过程塞进主上下文。任务完成后，Memory、Knowledge 和 Experience 分别持久化；相似任务先匹配经验 Description，再按需读取完整 Content。
>
> 所以这个系统的核心不是某个单点功能，而是几个边界的组合：Prompt 是信息入口，Tool 是能力入口，Permission 是执行边界，Context 是短期工作区，Memory/Knowledge/Experience 是用途不同的长期资产，Skill/Sub-agent/MCP 是能力扩展和上下文隔离机制。

## 第二部分：面试问答与追问补充

### Q1：面试官问：你如何用一条主线介绍整个项目？

我会按一次用户请求的生命周期讲。用户输入先到 CLI，CLI 判断是命令还是普通请求；普通请求进入 `Agent.chat()`，Agent 准备 prompt、动态上下文和工具列表；模型生成文本或工具调用；工具调用先过权限系统，再分发到本地工具、MCP、Skill 或子 Agent；结果回填后继续循环；过程中上下文管理负责防止 token 膨胀；任务完成后，Experience、Memory 和 Knowledge 按不同结构持久化并按需复用。
...
Memory 保存长期偏好和项目事实，比如用户喜欢中文回答、某个项目的约定。Knowledge 保存外部文档和资料，分块检索后返回局部依据。Experience 来自任务轨迹沉淀，完整 Markdown 保留有序排查过程，Description 负责召回，`experience_show` 负责读取 Content 全文。Skill 是可复用工作流 prompt，指导 Agent 以后遇到这类任务怎么做。
...
渐进式披露的做法是先给索引和摘要，真正需要时再加载详情。比如 Skill 先披露名称和描述，调用时才读取完整 `SKILL.md`；Knowledge 通过 query 返回相关片段；Experience 先基于 Description 召回候选，再读取命中的完整 Content；自定义 Agent 先披露类型描述，调用时才加载完整 prompt。这样能控制 token 成本，也能避免经验步骤因普通分块而失去连续性。
