...
工具定义本身不保存硬权限。`tool_definitions` 主要告诉模型工具名称、用途和参数 schema，让模型知道如何提出工具调用；真正的执行权限由 `check_permission()` 在工具调用事件发生后判断。`.claude/settings.json` 也不是必须存在，没有它时系统仍然会走默认权限策略、Plan Mode 限制、危险动作确认和工具内部保护。如果工具定义和 settings 看起来冲突，要以 settings 和权限网关为准：工具“可见”不等于工具“可执行”，模型可以请求调用，但宿主程序可以拒绝并把拒绝结果回填给模型。
...
这里还要区分“工具定义”和“权限定义”。工具定义在 `tools.py` 里描述 name、description 和 input schema，它决定模型能不能看见这个工具、知道怎么传参；权限定义在 `.claude/settings.json` 或 `~/.claude/settings.json` 里，决定某个具体工具调用事件能不能执行。settings 文件不是必需的，没有它时只是没有自定义 allow/deny，系统仍然有默认权限模式和危险动作检测。发生冲突时，权限网关优先：工具可见不代表可执行，命中 deny 的工具调用会被拒绝并作为 tool result 返回模型。
...
会话级白名单发生在 `confirm` 分支里。Python 版用 `self._confirmed_paths: set[str]` 保存本轮 Agent 实例中已经批准过的确认消息，例如 `rm -rf /tmp/demo`、`write new file: /path/to/file.py`、`edit non-existent file: /path/to/missing.py`。如果同一会话中再次出现完全相同的 `perm["message"]`，Agent 不再调用确认回调，直接继续执行；如果命令或路径有任何变化，就会作为新的确认项重新询问。这个白名单只在内存里，不写入 Session，也不写入 `CLAUDE.md` 或 settings，重启后失效。Auto Mode 的 confirm 不缓存，因为它的 message 是分类器风险原因，不是稳定的操作标识。

```mermaid
flowchart TD
    A[权限结果为 confirm] --> B{Auto Mode confirm}
    B -- 是 --> C[不使用会话白名单]
    B -- 否 --> D{确认消息已在白名单}
    D -- 是 --> E[跳过确认并执行]
    D -- 否 --> F[调用用户确认回调]
    C --> F
    F -- 用户拒绝 --> G[返回拒绝 tool_result]
    F -- 用户批准 --> H{是否可缓存}
    H -- 是 --> I[加入会话级白名单]
    H -- 否 --> J[不缓存]
    I --> K[执行工具]
...
### Q4A：面试官问：工具定义本身会定义权限吗？

不会。工具定义主要包括工具名、描述和参数 schema，作用是让模型知道“有哪些工具、什么时候用、怎么传参”。它可以通过 description 引导模型少误用工具，但这只是软约束。

真正的硬权限发生在工具调用事件之后，由 `check_permission()` 根据 permission mode、settings allow/deny、Plan Mode 和危险动作检测决定。工具可见不等于可执行，模型可以请求调用，宿主程序仍然可以拒绝。

### Q4B：面试官问：`.claude/settings.json` 是必须的吗？和工具定义冲突怎么办？

不是必须的。没有 settings 文件时，系统只是没有用户自定义 allow/deny，仍然有默认策略：只读工具默认允许，Plan Mode 禁止写和 shell，危险 shell、新建文件、编辑不存在文件会触发确认。

如果 settings 和工具定义看起来冲突，以 settings 和权限网关为准。工具定义决定模型能不能看见工具，settings 决定具体工具调用能不能执行。deny 最高优先级，可以覆盖默认 allow、settings allow，也能限制 `bypassPermissions`。
...
### Q5A：面试官问：会话级白名单到底白名单了什么？

它白名单的不是工具名，也不是路径对象，而是本次确认的 `perm["message"]` 字符串。比如危险 shell 的 message 是命令本身，新建文件可能是 `write new file: ...`，编辑不存在文件可能是 `edit non-existent file: ...`。

用户在当前会话里确认一次后，相同 message 再出现就不再重复询问。这个白名单只存在内存里，不写入 Session，也不写入 `.claude/settings.json` 或 `CLAUDE.md`，重启后失效。

### Q5B：面试官问：为什么会话白名单不持久化？

因为用户确认一次危险动作通常只代表当前上下文下同意，不代表以后所有会话都同意。持久化会把临时授权扩大成长期授权，风险更高。

如果用户确实想长期放行，应显式写入 `.claude/settings.json` 的 allow 规则；这样权限来源清楚、可审查，也能被 deny 规则覆盖。
