...
Memory 和 Knowledge 最容易混淆。Memory 是少量、项目级、用户化的长期提醒，规模通常不大，依赖 frontmatter 的 `name/description/type` 和 side query 做轻量选择；Knowledge 是外部文档知识库，文档可能很长，需要切分 chunk、建索引、做语义检索和关键词检索，再把命中的证据片段注入上下文。简单说，Memory 帮 Agent “记得用户和项目习惯”，Knowledge 帮 Agent “查到外部资料证据”。所以不应该把大段技术文档塞进 Memory，也不应该把用户讲解偏好放进 Knowledge。

Memory 和 Experience 也不同。Memory 通常是用户或模型主动沉淀的小条长期事实；Experience 是从任务轨迹里提炼出来的可复用工程流程。Experience 的依据是 `TaskJournal` 这类事件日志，因为聊天历史可能已经被 compact 或 snip 改写；生成后的 experience markdown 会进入 Knowledge，用后续检索召回。也就是说，Memory 偏“长期偏好和状态”，Experience 偏“做事方法和踩坑经验”。

Memory 和 Session 的边界是生命周期。Session 保存当前会话完整消息流，用于 resume；Memory 跨会话存在，但只保存少量长期价值信息。当前任务中刚读过的文件、刚失败的测试、刚做出的临时计划，应该留在 Session 或 Experience Journal，不应该立刻写进 Memory。否则 Memory 会变成聊天垃圾桶，召回质量会迅速下降。

Memory 和 `CLAUDE.md` 的边界是规则来源。`CLAUDE.md` 更适合项目级明确规范，例如团队要求、仓库约定、固定开发流程；Memory 更适合个体化偏好和阶段性提醒。Prompt 里明确要求不要保存已经在 `CLAUDE.md` 里的内容，因为同一规则如果同时存在两个来源，后续修改时容易不一致。

可以把四层关系记成一句话：**Session 记当前，Memory 记长期偏好和项目提醒，Knowledge 查外部证据，Experience 沉淀任务流程。**

```mermaid
flowchart TD
    A[Agent 需要上下文] --> B{信息类型}
    B -- 当前对话状态 --> C[Session]
    B -- 用户偏好/反馈/项目提醒 --> D[Memory]
    B -- 外部文档证据 --> E[Knowledge]
    B -- 可复用任务流程 --> F[Experience]
    C --> G[随会话 resume 和 compact]
    D --> H[Markdown 文件 + MEMORY.md 索引]
    E --> I[chunk + index + knowledge_search]
    F --> J[TaskJournal 提炼后写入 Knowledge]
```

## MEMORY.md 的具体内容与写入时机

`MEMORY.md` 是 memory 目录下的自动索引文件，不是真正的 memory 正文。它只记录每个 memory 文件 frontmatter 里的 `name`、`type`、`description` 和文件名，格式类似：

```markdown
# Memory Index

- **[用户讲解偏好](user_explanation_preference.md)** (user) — 用户希望用中文、按 Python 版代码、按事件流讲解
- **[项目解析规则](feedback_project_analysis_style.md)** (feedback) — 讲解时要关联 Agent Loop、工具、权限和上下文
- **[当前学习目标](project_interview_prep.md)** (project) — 当前项目用于学习 mini Claude Code 并准备面试
```

这些行来自单个 memory 文件的 frontmatter。例如：
...
`MEMORY.md` 会在两类事件后更新。第一类是程序内部调用 `save_memory()` 或 `delete_memory()`，它们都会调用 `_update_memory_index()` 重建索引。第二类是模型通过 `write_file` 写入 memory 目录下的普通 `.md` 文件时，工具层 `_auto_update_memory_index()` 会自动发现路径属于 memory 目录，并重建 `MEMORY.md`。如果只是普通聊天、只是召回 memory、只是执行 `/memory` 查看列表、写入的不是 memory 目录、写入的是 `MEMORY.md` 本身，索引都不会因此自动更新。

这个设计的关键是：模型不应该手动编辑 `MEMORY.md`。索引应该由真实 memory 文件生成，否则会出现索引里有条目但文件不存在，或者文件已更新但索引仍是旧描述的情况。

## 写入、更新、删除的判断边界

当前 Python 版没有一个硬编码的“什么时候写 Memory”的分类器。模型是否写入，主要来自 prompt 的软指令和用户意图。强写入信号是用户明确说“记住”“以后都按这个来”“下次继续沿用这个偏好”；中等信号是用户反复纠正同一类行为，适合沉淀为 `feedback`；弱信号是当前任务中出现长期项目决策，模型可以考虑写入 `project`，但不应该把每个临时步骤都写进去。

更新判断也主要靠语义。如果新信息修正的是同一个长期事实，就应该更新旧 memory，而不是新建。例如用户之前要求“讲 TypeScript 版本”，后来明确改成“之后都拿 Python 版本讲”，这是同一偏好的替换，应该更新 `user` 或 `feedback` memory。若新信息是另一条独立偏好，比如“面试问答要更真实”，则可以新建或合并到相关 feedback 文件。生产系统最好有显式 `memory_update`，要求模型说明更新原因、旧值、新值和影响范围。

删除判断更保守。当前有 `delete_memory(filename)` 函数，但 CLI 只暴露 `/memory` 列表，没有 `/memory delete`，工具系统也没有专门的 `memory_delete`。因此常规 Agent 流程不会自动删除 memory。旧 memory 只会在召回时被加上 freshness warning，提醒模型它可能过期。生产系统里删除长期记忆应该要求用户确认，并支持软删除、版本历史或恢复。

```mermaid
flowchart TD
    A[出现一条新信息] --> B{是否明确要求记住}
    B -- 是 --> C[强写入信号]
    B -- 否 --> D{是否长期影响后续行为}
    D -- 否 --> E[留在 Session 不写 Memory]
    D -- 是 --> F{是否修正已有 Memory}
    F -- 是 --> G[读取旧 Memory 并更新]
    F -- 否 --> H[按类型新建 Memory]
    C --> I{是否已有同类 Memory}
    I -- 是 --> G
    I -- 否 --> H
    G --> J[write_file/edit_file 写入 memory 文件]
    H --> J
    J --> K[自动刷新 MEMORY.md]
```
...
单文件注入最多 `4096` bytes，指的是 memory 文件被 side query 选中后，真正注入给主模型前会做大小限制。代码先读取完整 memory 文件，如果 `len(content.encode()) > MAX_MEMORY_BYTES_PER_FILE`，就只把前部内容和 `[... truncated, memory file too large ...]` 提示放入上下文。磁盘上的 memory 文件不会被修改，完整内容仍然存在。

这不是删除，也不是压缩保存，而是“召回注入时的上下文裁剪”。它的目的是防止某个 memory 文件写得太长，一次召回就占掉大量上下文。风险是当前实现只保留开头，不保留尾部；如果关键内容在后半部分，主模型可能看不到。因为注入 header 里带有 memory 文件路径，模型理论上可以再用 `read_file` 读取完整文件。生产化可以改成 head-tail 截断、结构化摘要，或者限制 memory 文件本身保持短小。

```mermaid
flowchart TD
    A[side query 选中某个 memory] --> B[读取完整 memory 文件]
    B --> C{UTF-8 编码是否超过 4096 bytes}
    C -- 否 --> D[完整注入]
    C -- 是 --> E[注入前部内容 + truncated 提示]
    E --> F[磁盘原文件保持完整]
    D --> G[包装为 system-reminder]
    F --> G
    G --> H[追加到最后一条 user message]
    H --> I{后续需要完整内容}
    I -- 是 --> J[用 read_file 读取 memory 原文件]
    I -- 否 --> K[使用当前注入内容]
```
...
### Q5A：面试官问：`MEMORY.md` 里具体有什么？什么时候会写入？

`MEMORY.md` 里只有索引信息，一般是 `# Memory Index`，下面每行包含 memory 的 `name`、文件链接、`type` 和 `description`。它不包含 memory 正文，所以不能把它当成完整事实来源。

写入时机有两类：程序内部调用 `save_memory()` 或 `delete_memory()` 会重建索引；模型通过 `write_file` 写入 memory 目录下的普通 `.md` 文件时，`_auto_update_memory_index()` 也会自动重建索引。普通聊天、召回 memory、执行 `/memory` 查看列表、写入非 memory 目录，都不会自动更新它。

### Q5B：面试官问：为什么不让模型手动维护 `MEMORY.md`？

因为 `MEMORY.md` 是派生索引，不是 source of truth。让模型手动改索引，很容易出现索引和真实文件不一致，比如索引里有条目但文件不存在，或者文件内容更新了但 description 还是旧的。

更稳妥的方式是所有 memory 文件作为事实源，索引由程序自动生成。这样写入、更新、删除都只有一个真实来源。
...
### Q13A：面试官问：单个 Memory 注入最多 4096 bytes 是什么意思？

意思是 memory 文件被召回后，注入给主模型前会做大小限制。如果文件 UTF-8 编码长度超过 4096 bytes，当前实现会只把前部内容和 truncated 提示放进上下文。磁盘上的原始 memory 文件不会被修改，完整内容仍然存在。

这是上下文裁剪，不是数据删除。风险是当前实现只保留开头，不保留尾部，如果关键内容在后半段，模型可能看不到。生产化可以改成 head-tail 截断、结构化摘要，或者要求 memory 文件保持短小。

### Q13B：面试官问：如果截断后模型还需要完整 Memory 怎么办？

注入的 header 里包含 memory 文件路径，模型可以再用 `read_file` 读取完整文件。所以它是可恢复的上下文裁剪。

但这个恢复依赖模型意识到“当前内容被截断了”。更稳的生产方案是为 memory 提供专门读取工具，或者在截断提示里明确给出可调用的 memory id。
