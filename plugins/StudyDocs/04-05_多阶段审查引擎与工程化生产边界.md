...
本章沿用总览章的 `CASE-7A91`：`CLAIM-R1` 在 L3 已由拒绝路径证据判为 consistent，`CLAIM-R2` 因为“拒绝时是否记录安全审计”证据不足而进入 L4。后文每个阶段都只围绕 `CLAIM-R2` 增加信息。这样理解 L4 时，关注点不是模型连续回答了四次，而是同一个未知量如何依次变成假设、gap、observation 和最终 finding。
