...
class ExpenseService:
    ...
    def submit(
        ...
        # 串行重复请求可以去重，但查询与插入是两个独立操作，不属于同一事务。
        expense = self.repository.find_by_request(actor.user_id, request_id)
        created = expense is None
        if expense is None:
            expense = self.repository.create_unchecked(factory())
        if created:
    ...
    def process(self, expense_id: str) -> PaymentOutcome:
        ...
        for attempt in range(1, 4):
            try:
                ...
                if attempt < 3:
                    self.sleeper.sleep(float(2 ** (attempt - 1)))
