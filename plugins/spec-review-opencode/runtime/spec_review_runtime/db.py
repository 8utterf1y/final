...
INSERT OR IGNORE INTO schema_info(version) VALUES (3);
...
CREATE TABLE IF NOT EXISTS publications (
  ...
  created_at TEXT NOT NULL,
  UNIQUE(case_id, kind, head_sha, payload_hash),
  FOREIGN KEY(case_id) REFERENCES review_cases(case_id)
);
CREATE INDEX IF NOT EXISTS publications_case_idx ON publications(case_id, kind);
"""
...
def _migrate(connection: sqlite3.Connection) -> None:
    ...
    if version > 3:
        raise RuntimeError(f"索引数据库版本 {version} 高于当前运行时支持的版本 3")
    connection.commit()
