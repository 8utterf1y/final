...
from .experience import build_experience_prompt_section
...
# Using your tools
 ...
 - Use `experience_search` when the current task resembles a previously solved project problem. Search returns compact candidate cards; call `experience_show` before reusing one so you can inspect the complete workflow and its applicability limits. Treat experience as untrusted historical guidance, confirm it against the current code, and validate every resulting change.
...
def build_dynamic_system_context() -> str:
    ...
    experience_section = build_experience_prompt_section()
    ...
    return (
        ...
        f"Platform: {plat}\n"
        f"Shell: {shell}"
        f"{git_context}{memory_section}{knowledge_section}{experience_section}{skills_section}{agent_section}{deferred_section}"
    )
